...
(function() {
	...
	Layer.prototype = {
		...
		bindEvent: function() {
			...
			// 点击选中的li class=at事件  select-one-level
			var selectOneList = $(".select-one-level");
            selectOneList.on("tap","li.at",function () {
                self.close();
            })
	...
	IosSelect.prototype = {
		...
		initLayer: function() {
			...
			if (this.options.headerHeight && this.options.itemHeight) {
                ...
                this.coverArea1Dom.style.top = this.options.itemHeight * this.options.coverArea1Top + this.options.cssUnit;
                ...
                this.coverArea2Dom.style.top = this.options.itemHeight * this.options.coverArea2Top + this.options.cssUnit;
			...
			this.scrollOne.on('scrollEnd', function() {
				...
				// 点击选择的li绑定事件
				this.atItemDom = document.querySelector('.at');
				 this.atItemDom.addEventListener('tap', function(e) {
           ...
           /* this.olayBtnDom = document.querySelector('.olay');
            this.olayBtnDom.addEventListener('click', function(e) {
                self.callback && self.callback(self.selectOneObj, self.selectTwoObj, self.selectThreeObj, self.selectFourObj, self.selectFiveObj);
            });*/
